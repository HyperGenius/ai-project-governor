...
このドキュメントは、本プロジェクト（AI Project Governor）の開発を担当するAI Agent（GitHub Copilot Agent等）に向けた、アーキテクチャ、コーディング規約、テスト方針、および開発プロセスを定義するものです。

## 1. プロジェクト概要

**AI Project Governor** は、エンジニアの「報告業務（日報・週報）」を自動化し、プロジェクト管理を効率化するAIアシスタントツールです。

* **Core Value**: "事実は人間が、建前（報告フォーマット）はAIが"。入力は箇条書きで済ませ、出力は完璧なビジネス文書へ変換する。
* **Current Phase**: Phase 2 (User Value Enhancement) - 対話型要件定義やカスタマイズ性の強化。
...
1. **RLS (Row Level Security)**: すべてのテーブルでRLSを有効化する。`tenant_id` による分離を徹底する。
...
* **Rules**:
* DB接続や外部API（Gemini, Supabase）は **必ずMock化 (`unittest.mock`)** する。
* 特にAI生成ロジックは、実際のAPIを叩かずにMockレスポンスで検証する（コストと速度のため）。
...
現在の開発フェーズでは、以下の機能実装が優先されます。

1. **対話型プロジェクト要件定義 (Interactive Scoping)**
* AIがユーザーに質問を行い、WBSの精度を高めるチャットUIの実装。


2. **プロンプトカスタマイズ**
* ユーザーごとの「AI人格」や「口調」の設定機能。


3. **ガントチャートの高度化**
* タスク依存関係の線描画や、マイルストーンの可視化。
